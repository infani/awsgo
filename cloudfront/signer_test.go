...
func TestSignUrl(t *testing.T) {
	...
	s3.PutFile(config.Bucket, key, bytes.NewReader([]byte("#EXTM3U")));
	rawURL := url.URL{
		Scheme: "https",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := SignUrl(tt.args.url, tt.args.expire); !strings.Contains(got, tt.want) {
...
func TestSignCookie(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := SignCookie(tt.args.url, tt.args.expire)
